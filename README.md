...
- Comparing the raw wall times instead would give 8.2×, which is the same two runs counted over
...
I went back through the committed logs in September 2026 and re-derived every number on this page
from them. Three needed changing, and the reasoning behind them is worth keeping.

| Claim | What the logs show | Now reads |
|---|---|---|
| GPU is 8.2× the 256-core CPU run | The CPU run simulated 1000 s, the GPU runs 500 s, same `dt` — twice the timesteps on one side | **≈4.1×**, CPU times normalised per simulated second |
| Output is parallel NetCDF | `module_output.F90` gathers each rank's slab onto rank 0 and writes it there | NetCDF via rank-0 aggregation |
| The bottleneck is branching | `perf`'s 74.6% bad-speculation figure came from multiplexed counters and contradicts the 0.05% branch-miss rate in the same run | Memory isn't the limit at this size; the stall isn't yet identified |

**The normalisation bug is the one I'd flag to anyone benchmarking.** The two runs came from different
SLURM scripts with different `sim_time`, and the comparison was built from the two summary lines
rather than from the run configurations. A per-simulated-second column in the timer output makes that
class of mistake impossible, and that's the fix I'd apply first.

**The `perf` one is a cross-checking lesson.** A single tool reported a striking number that matched
the story we already believed — a stencil kernel with branchy boundary handling. The branch-miss
counter in the *same output* disagreed with it. Two counters from one run, pointing opposite ways, is
the cheapest contradiction there is to catch, and it only needs someone to read the whole output.

Nailing down the real stall needs a non-multiplexed top-down measurement, one event group per run.
The multi-GPU compute time also flattens at ~13 s from 12 GPUs on, which points at per-kernel launch
and sync overhead rather than the halo exchange — that's the next thing worth profiling.
